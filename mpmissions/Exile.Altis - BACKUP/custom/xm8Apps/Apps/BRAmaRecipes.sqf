...
_GoBackBtn ctrlSetText localize "STR_HOPE_NATION_BASEMARKER_GOBACK";
...
_GoCraftBtn ctrlSetText localize "STR_HOPE_NATION_CRAFTING_CRAFT_BUTTON";
...
_RecipePic ctrlSetText "custom\xm8Apps\images\crafting_app.paa";
...
for '_j' from 0 to (count (missionConfigFile >> "CfgCraftingRecipes"))-1 do
{
	...
	_RecipeCategory = localize "STR_HOPE_NATION_CAT_UNCATEGORISED";
	if!(_RecipeCategory in _RecipeCategories)then{_RecipeCategories pushBack _RecipeCategory;};
...
lbClear 5502;
lbClear 5501;
(_display displayCtrl 5507) ctrlSetText "custom\xm8Apps\images\crafting_app.paa";
...
for '_j' from 0 to (count (missionConfigFile >> "CfgCraftingRecipes"))-1 do
{
	...
	{
		if (_SelectedCategory == localize "STR_HOPE_NATION_CAT_UNCATEGORISED" && _RecipeCategory == "") then
		{
			_lbsize = lbSize (_display displayCtrl 5501);
			(_display displayCtrl 5501) lbAdd Format["%1",_currentRecipeName];
			(_display displayCtrl 5501) lbSetPicture [_lbsize,_recipePicture];
