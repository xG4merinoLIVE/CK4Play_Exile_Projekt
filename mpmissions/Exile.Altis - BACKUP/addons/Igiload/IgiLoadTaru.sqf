...
if (hasInterface && !isDedicated) then {
        ...
        waituntil
        {
...

            {
                if (isnil {_x getVariable "IL_Taru_Action_Attach"}) then
                {
                    ...
                    IL_Taru_Action_Attach = _x addAction ["<img image='addons\Igiload\images\load.paa' /><t color=""#007f0e""> Attach the Pod</t>", "[""attach"",_this] call IL_Taru_Do_Action;", nil, 2, false, true, "",
                ...
                if (isnil {_x getVariable "IL_Taru_Action_Deattach"}) then
                {
                    ...
                    IL_Taru_Action_Deattach = _x addAction ["<img image='addons\Igiload\images\unload.paa' /><t color=""#ff0000""> Detach the Pod</t>", "[""deattach"",_this] call IL_Taru_Do_Action;", nil, 2, false, true, "",
                ...
                if (isnil {_x getVariable "IL_Taru_Action_Drop"}) then
                {
                    ...
                    IL_Taru_Action_Drop = _x addAction ["<img image='addons\Igiload\images\unload_para.paa' /><t color=""#b200ff""> Drop the Pod</t>", "[""drop"",_this] call IL_Taru_Do_Action;", nil, 2, false, true, "",
    ...
    IL_Taru_Transmission =
    {
        ...
        switch (_args select 0) do {
            ...
            case 'sound_attach': {
                 if (!(player in crew _helico)) exitWith{};
                 _soundToPlay = _soundPath + "addons\Igiload\sounds\attach.wss";
                 playSound3D [_soundToPlay, _helico, false, getPosATL _helico, 10, 1, 0];
            };
            case 'sound_deattach': {
                 if (!(player in crew _helico)) exitWith{};
                 _soundToPlay = _soundPath + "addons\Igiload\sounds\deattach.wss";
                 playSound3D [_soundToPlay, _helico, false, getPosATL _helico, 10, 1, 0];
            };
            case 'sound_drop': {
                 if (!(player in crew _helico)) exitWith{};
                 _soundToPlay = _soundPath + "addons\Igiload\sounds\drop.wss";
                 playSound3D [_soundToPlay, _helico, false, getPosATL _helico, 10, 1, 0];
            };
        };
    };
